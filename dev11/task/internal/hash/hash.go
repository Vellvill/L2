...
func (h *Hash) Create(id int64, desc string, date time.Time) (*model.Event, error) {
	if user, ok := h.hash[id]; ok {
		event := model.NewEvent(date, desc)
...
func (h *Hash) Update(id int64, desc string, date time.Time, eventTime time.Time, newTime time.Time) (*model.Event, error) {
	if user, ok := h.hash[id]; ok {
...
func (h *Hash) Delete(id int64, desc string, date time.Time, eventTime time.Time) error {
	if user, ok := h.hash[id]; ok {
		if j, ok := user[date]; ok {
			for _, v := range j {
				if v.Time == eventTime {
